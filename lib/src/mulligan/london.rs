...
mod tests {
  ...
  // In the London mulligan, we always pack our opening hand with lands
  // so if we mulligan down to 2, we should always have 2 lands and draw
  // agonizing remorse on t2
  #[test]
  fn london_tap_test_0() {
    let code = "
    1 Agonizing Remorse # T = 0
    1 Cinder Barrens
    1 Mountain
      ";
  ...
  // The london mulligan kept lands are essentially random. We expect to keep
  // the Mountain 50% of the time, and the Cinder Barrens the other 50%, so we
  // should expect a tapped result 50% of the time.
  #[test]
  fn london_tap_test_1() {
    let code = "
    1 Agonizing Remorse # T = 0
    1 Cinder Barrens
    1 Mountain
    ...
    let sim = Simulation::from_config(&SimulationConfig {
      run_count: runs,
      draw_count: draws,
      mulligan: &London::always(1),
      deck: &deck,
      on_the_play: true,
    });
    let obs = sim.observations_for_card(card);
    assert!(f64::abs(obs.p_tapped() - 0.16) < 0.03);
    assert!(f64::abs(obs.p_tapped_given_cmc() - 0.5) < 0.03);
  }

  // If we're on the draw ...
  #[test]
  fn london_tap_test_2() {
    let code = "
    1 Agonizing Remorse # T = 0
    1 Cinder Barrens
    1 Mountain
    ...
    let obs = sim.observations_for_card(card);
    assert!(f64::abs(obs.p_tapped() - 0.333) < 0.03);
    assert!(f64::abs(obs.p_tapped_given_cmc() - 0.333) < 0.03);
  }
}
